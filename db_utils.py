...
def update_milk_record(db: Session, record_data: dict):
    """Update existing milk record or create if not exists"""
    record = db.query(MilkRecord).filter(
        MilkRecord.date == record_data['date'],
        MilkRecord.animal_id == record_data['animal_id'],
        MilkRecord.session == record_data['session']
    ).first()
    
    if record:
        for key, value in record_data.items():
            if key != 'date' and key != 'animal_id' and key != 'session':
                setattr(record, key, value)
        db.commit()
        db.refresh(record)
        return record
    else:
        return add_milk_record(db, record_data)
