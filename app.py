...
def get_current_milking_session():
    """Auto-detect current milking session based on Kolkata time"""
...
def get_last_n_milk_sessions(animal_id, n=4):
    """Get last N milk records for a specific animal, sorted chronologically"""
    if st.session_state.milk_records.empty:
        return []
    
    animal_records = st.session_state.milk_records[
        st.session_state.milk_records['animal_id'] == animal_id
    ].copy()
    
    if animal_records.empty:
        return []
    
    animal_records['date'] = pd.to_datetime(animal_records['date'])
    
    animal_records['session_order'] = animal_records['session'].apply(
        lambda x: 1 if 'Evening' in x else 0
    )
    
    animal_records = animal_records.sort_values(
        ['date', 'session_order'], 
        ascending=[False, False]
    )
    
    last_n = animal_records.head(n)
    
    sessions_data = []
    for _, row in last_n.iterrows():
        date_str = row['date'].strftime('%d %b')
        session_str = 'M' if 'Morning' in row['session'] else 'E'
        sessions_data.append({
            'label': f"{date_str}-{session_str}",
            'value': row['yield_litres'],
            'date': row['date'].date(),
            'session': row['session']
        })
...
def milk_collection_sales():
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["⚡ Quick Milk Entry", "Record Collection", "View Records", "Sales Analytics"])
    
    with tab1:
        st.subheader("⚡ Quick Daily Milk Entry")
        ...
        if not st.session_state.animals.empty:
            ...
            if lactating_cows.empty:
                st.warning("No active adult cows found. Please register animals first.")
            else:
                current_session_info = get_current_milking_session()
                
                use_previous_date = st.checkbox("📅 Enter Previous Date Data")
                
                if use_previous_date:
                    col1, col2 = st.columns(2)
                    with col1:
                        selected_date = st.date_input("Select Date", value=datetime.now().date() - timedelta(days=1))
                    with col2:
                        selected_session = st.selectbox("Select Session", ["Morning (AM)", "Evening (PM)"])
                    
                    entry_date = selected_date
                    entry_session = selected_session
                    entry_label = "Morning" if "Morning" in selected_session else "Evening"
                else:
                    entry_date = current_session_info['date']
                    entry_session = current_session_info['session']
                    entry_label = current_session_info['session_label']
                
                st.markdown(f"### 📊 {entry_date.strftime('%d %B %Y')} - {entry_label} Milking")
                
                if not current_session_info['is_active_time'] and not use_previous_date:
                    st.info("ℹ️ Note: Current time is outside typical milking hours (3-6 AM or 3-6 PM)")
                
                existing_records = {}
                if use_previous_date and not st.session_state.milk_records.empty:
                    day_records = st.session_state.milk_records[
                        (pd.to_datetime(st.session_state.milk_records['date']).dt.date == entry_date) &
                        (st.session_state.milk_records['session'] == entry_session)
                    ]
                    for _, record in day_records.iterrows():
                        existing_records[record['animal_id']] = record['yield_litres']
                
                st.markdown("---")
                
                milk_entries = {}
                
                header_cols = st.columns([1.5, 2, 1.5, 0.8, 0.8, 0.8, 0.8])
                header_cols[0].markdown("**Cow ID**")
                header_cols[1].markdown("**Name**")
                header_cols[2].markdown(f"**{entry_date.strftime('%d %b')}-{entry_label[0]}** (L)")
                header_cols[3].markdown("**-1**")
                header_cols[4].markdown("**-2**")
                header_cols[5].markdown("**-3**")
                header_cols[6].markdown("**-4**")
                
                st.markdown("---")
                
                for idx, cow in lactating_cows.iterrows():
                    cow_id = cow['animal_id']
                    cow_name = cow['name'] if cow['name'] else '-'
                    
                    cols = st.columns([1.5, 2, 1.5, 0.8, 0.8, 0.8, 0.8])
                    
                    cols[0].markdown(f"`{cow_id}`")
                    cols[1].markdown(cow_name)
                    
                    default_value = existing_records.get(cow_id, 0.0)
                    milk_yield = cols[2].number_input(
                        f"Milk for {cow_id}",
                        min_value=0.0,
                        max_value=50.0,
                        step=0.1,
                    ...
                    if milk_yield > 0:
                        milk_entries[cow_id] = milk_yield
                    
                    last_sessions = get_last_n_milk_sessions(cow_id, n=4)
                    
                    for i in range(4):
                        if i < len(last_sessions):
                            cols[3 + i].markdown(f"<div style='text-align: center; color: #666;'>{last_sessions[i]['value']:.1f}</div>", unsafe_allow_html=True)
                        else:
                            cols[3 + i].markdown("<div style='text-align: center; color: #ccc;'>-</div>", unsafe_allow_html=True)
                
                st.markdown("---")
                
                col1, col2, col3 = st.columns(3)
                with col1:
                    default_usage = st.selectbox(
                        "Default Usage",
                        ["Sold", "Consumed (Home)", "Calf Feeding", "Wastage"],
                        help="This will be applied to all entries"
                    )
                with col2:
                    default_price = st.number_input(
                        ...
                        step=1.0,
                        value=50.0,
                        help="Applied only for 'Sold' milk"
                    )
                with col3:
                    st.markdown("")
                    st.markdown("")
                ...
                if st.button("💾 Save All Entries", type="primary", use_container_width=True):
                    if milk_entries:
                        ...
                        try:
                            ...
                            for cow_id, yield_litres in milk_entries.items():
                                if cow_id in existing_records:
                                    db_utils.update_milk_record(db, {
                                        'date': entry_date,
                                        'animal_id': cow_id,
                                ...
                                success_count += 1
                                total_milk += yield_litres
                                
                                if default_usage == "Sold":
                                    revenue = yield_litres * default_price
                                    ...
                                    transaction_data = {
                                        ...
                                        'subcategory': cow_id,
                                        'amount': revenue,
                                        'description': f"Milk sale - {yield_litres}L @ ₹{default_price}/L",
                                        'reference_id': cow_id,
                                        'notes': ''
                                    }
                                    db_utils.add_financial_transaction(db, transaction_data)
                            
                            st.success(f"✅ Successfully recorded {success_count} entries! Total: {total_milk:.1f}L" + 
                                     (f" | Revenue: ₹{total_revenue:.2f}" if default_usage == "Sold" else ""))
                            load_data_from_db()
                            st.rerun()
                        except Exception as e:
                            st.error(f"Error saving milk records: {str(e)}")
                            db.rollback()
                        finally:
                            db.close()
                    else:
                        st.warning("Please enter milk yield for at least one cow.")
        else:
            st.warning("Please register animals first in Animal Management.")
    ...
    with tab2:
        ...
        if not st.session_state.animals.empty:
            ...
            col1, col2, col3 = st.columns(3)
            with col1:
                collection_date = st.date_input("Date*", value=datetime.now().date(), key="individual_date")
                animal_id = st.selectbox("Cow ID*", lactating_cows, key="individual_cow")
            
            with col2:
                session = st.selectbox("Session*", ["Morning (AM)", "Evening (PM)"], key="individual_session")
                yield_litres = st.number_input("Milk Yield (litres)*", min_value=0.0, step=0.1, key="individual_yield")
            
            with col3:
                usage = st.selectbox(
                    "Usage*",
                    ["Sold", "Consumed (Home)", "Calf Feeding", "Wastage"],
                    key="individual_usage"
                )
                price_per_litre = st.number_input("Price per Litre (₹)", min_value=0.0, step=1.0, value=50.0, key="individual_price")
            
            notes = st.text_area("Notes", key="individual_notes")
